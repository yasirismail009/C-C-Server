  ...
  return (
    <Card>
      ...
      <div className="flex flex-row">
             <div> <h4 ><b>Contacts</b></h4>
            <p>
              All Contacts found on the machines
            </p>
            </div>
   {devices.devices_data === null || devices.devices_data === undefined ? null :
      ...
      <FormControl fullWidth >
       ...
       { devices.devices_data.map((val, key)=>(
           <MenuItem onClick={()=>{ ContactsData(val.device_key); DeviceKeyChange(val.device_key)}} value={val.device_name} key={key}>{val.device_name}</MenuItem>
         ))} 
      ...
      <CardBody>
      <Grid  container rowSpacing={1} columnSpacing={{ xs: 3, sm: 4, md: 3,lg:2 }}>
     {
     contacts.contacts_data === null || contacts.contacts_data === undefined ? null 
     :
       ...
       <Grid key={key} item lg={2} md={2} xs={12}>
    	...
    	<CopyToClipboard text=  {val.phone_number}>
								<IconButton>
									...
									<Icon color="action" style={{fontSize:40}} className="text-20 text-gray-400">
										content_copy
									</Icon>
								</IconButton>
							</CopyToClipboard>
              </div>
      <CardContent style={{justifyContent:'center', textAlign:'center' , alignContent:'center'}}>
        <Typography sx={{ fontSize: 20 }} color="text.secondary" gutterBottom>
       <PersonPinIcon color="primary" style={{fontSize:40}}/>
        </Typography>
