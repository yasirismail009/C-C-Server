    ...
    <GridContainer>
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          <CardHeader color="primary">
            <div>
              <div>
            <h4><b>Photo Gallerey</b></h4>
            <p>
             Photos Got through malware
            </p>
           </div>
      ...
      <FormControl fullWidth >
       ...
       { devices.devices_data.map((val, key)=>(
           <MenuItem onClick={()=>{ PhotosData(val.device_key); DeviceKeyChange(val.device_key)} } value={val.device_name} key={key}>{val.device_name}</MenuItem>
         ))} 
          ...
          <CardBody>
          <Grid container rowSpacing={1} columnSpacing={{ xs: 3, sm: 4, md: 3,lg:5 }}>
     {photos.pictures_data === null || photos.pictures_data === undefined ? null :
     photos.pictures_data.map((val, key)=>(
     <Grid item key={key} lg={2} md={3} xs={12}>
         <div className="img_wrap"  >
      <img   
        
   src={val.image_path} width="100%" height="140"/></div>
        </Grid> 
        ))
        }  
      </Grid>
